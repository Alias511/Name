$(function(){
  ...
  modal: true
   ...
   // перетакскивание элементов

   $("#drag1,#drag2,#drag3").draggable({
    revert: true,
    helper: 'clone',
    opacity: 0.5,
    zIndex: 100 // в папку img добавили trashcan-empty-icon.png, trashcan-full-icon.png
  });
...
$('#trash').droppable({
  ...
  drop: function (event, ui) {
    ui.helper.hide('explode'); //ui.helper - доступ к элементу
      $(this).attr('src', 'img/trashcan-full-icon.png');  // удаление в корзину, Drag'n'Drop работает в паре с Draggable
  
  }
});
//Sortable- сортируемые элементы
$('#sort1').sortable({
  connectWith: '#sort2',
   helper: 'clone',
    opacity: 0.5,
